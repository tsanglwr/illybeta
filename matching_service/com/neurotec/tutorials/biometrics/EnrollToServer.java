...
public final class EnrollToServer {

	private static final String DESCRIPTION = "Demonstrates how to enroll template to server";
	private static final String NAME = "enroll-to-server";
  ...
  private static final String WIN32_X86 = "Win32_x86";
    ...
    public static void initLibraryPath() {
       ...
       // if (Utils.isNullOrEmpty(jnaLibraryPath)) {
         if ("" == jnaLibraryPath || null == jnaLibraryPath) {
            ...
            System.setProperty("jna.library.path", String.format("%s%s%s", jnaLibraryPath, EnrollToServer.PATH_SEPARATOR, libraryPath.toString()));
        }
        System.setProperty("java.library.path",String.format("%s%s%s", System.getProperty("java.library.path"), EnrollToServer.PATH_SEPARATOR, libraryPath.toString()));
    ...
    public static String getLibraryPath() {
        ...
        int index = EnrollToServer.getWorkingDirectory().lastIndexOf(EnrollToServer.FILE_SEPARATOR);
        if (index == -1) {
            return null;
        }
        String part = EnrollToServer.getWorkingDirectory().substring(0, index);
        if (Platform.isWindows()) {
            if (part.endsWith("Bin")) {
                path.append(part);
                path.append(EnrollToServer.FILE_SEPARATOR);
                path.append(Platform.is64Bit() ? WIN64_X64 : WIN32_X86);
            }
        } else if (Platform.isLinux()) {
            index = part.lastIndexOf(EnrollToServer.FILE_SEPARATOR);
            if (index == -1) {
                return null;
            }
	...
	public static void main(String[] args) {
		EnrollToServer.initLibraryPath();
